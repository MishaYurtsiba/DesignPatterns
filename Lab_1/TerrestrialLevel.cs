...
namespace Lab_1
{
    internal class TerrestrialLevel:ILevel
    {
        public List<CreateNPC> listNPC { get; set; }
        public List<CreateDecorations> decorations { get; set; }
        public string Type { get; set; }
        public int Size { get; set; }
        public TerrestrialLevel(List<CreateNPC> _listNPC, List<CreateDecorations> decorations, string _Type, int _Size)
        {
        ...
        public string[,] CreateLevel()
        {
            ...
            for (int i = 0; i < Size; i++)
            {
                for (int j = 0; j < Size; j++)
                {
                    ...
                    {
                        if (i == decorations[k].x && j == decorations[k].y && decorations[k].Type != "Stone")
                        {
                            level[i, j] = "|";
                        }
                    }
